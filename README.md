...
The fourth journey, **Emerald Jungle**, follows a turquoise river through dense, humid rainforest. The river runs below the road on the camera side as a chain of terraced pools: each rocky lip spills over a cascade with a foaming plunge pool and drifting spray, and a low haze hangs over the water. Both banks are packed with faceted broadleaf crowns, leaning palms, ferns, broad-leaved plants, undergrowth, mossy boulders, and fallen logs, while buttressed emergent giants beside the road spread wide, flat crowns and hanging lianas over the asphalt so their shadows fall across it. The light is filtered rather than direct: a weak, green-tinted sun sits almost overhead with a strong green sky bounce and close humid fog, and forested crags, bumpy treetop hills, and two ranges of misty peaks close the valley. Number keys 1–4 also jump straight to a route.
...
- `src/world/jungle-route.js`, `src/world/jungle.js`: a river valley on the camera side with terraced pools whose lips, levels, and cascades are pure functions of world position, exact bank columns, road cuttings, forested crags, treetop-textured distant hills, and misty ranges. Scenery samples the rendered terrain mesh and keeps near-side trees below the camera's line of sight to the road; the same nine-chunk streaming budget and worker transfer apply.
- `src/world/jungle-assets.js`, `src/world/jungle-water.js`: lobed broadleaf and umbrella crowns, buttressed emergent trunks, leaning palms with creased fronds, ferns, broad leaves, grass tufts, boulders with baked moss, and shaders for the flowing turquoise river, cascade foam, and spray.
