...
The fourth journey, **Emerald Jungle**, follows a turquoise river through dense, humid rainforest. The river runs below the road on the camera side and always flows down the screen, so every waterfall faces the camera. It steps from pool to pool over rocky lips, each pouring a streaked, fraying sheet into foam and spray, with lily pads in the calm water. Where the road climbs too steeply for the river to follow, a mossy rock barrier overgrown with palms ends one reach, and the next is fed by a broad waterfall off the gorge wall. Along much of the route the road-side bank becomes that gorge: a mossy terrace ends in a wall of warm grey rock columns hung with vines, the water laps its foot, and a steel guardrail follows the road above. Creeks run down the far hillside to culverts under the road, meander across the terrace, and fall down the wall into the river. Both banks are packed with faceted broadleaf crowns, leaning palms, banana plants, ferns, broad-leaved plants, undergrowth, mossy boulders, and fallen logs, while buttressed emergent giants beside the road spread wide, flat crowns and curtains of lianas over the asphalt. The forest is not uniform: some stretches become palm groves, bamboo thickets, or tunnels of giant trees. The light is filtered rather than direct: a weak, green-tinted sun sits almost overhead with a strong green sky bounce and close humid fog, and blocky forested crags, bumpy treetop hills, and two ranges of peaks close the valley, with patches of mist drifting above the canopy. Number keys 1–4 also jump straight to a route.
...
- `src/world/jungle-route.js`, `src/world/jungle.js`: a river valley on the camera side whose pools are walked downstream from fixed anchor pools, so lips, barriers, and levels are pure functions of world position and the water never runs uphill; gorge walls, side falls, creeks, and forest zones are fixed in world space too, with exact bank columns, road cuttings, forested crags, treetop-textured distant hills, and misty ranges. Scenery samples the rendered terrain mesh and keeps near-side trees below the camera's line of sight to the road; the same nine-chunk streaming budget and worker transfer apply.
