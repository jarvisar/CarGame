...
- `src/world/jungle-route.js`, `src/world/jungle.js`: a continuous watershed with a positive grade and bounded pool-height variation, ensuring every step descends downstream. The jungle road follows the same grade smoothly. Pool widths, uneven lips, bank outcrops, side streams, and rock wakes are deterministic in world space. Extra terrain rows resolve waterfall sills and plunge basins. Scenery samples the rendered terrain mesh; the same nine-chunk streaming budget and worker transfer apply. Foliage batches that sprawl across the valley are halved along their longest axis until each part is small enough for the frustum test to reject, and the background forest layers stay out of the ambient-occlusion prepass. See [Jungle drawing cost](#jungle-drawing-cost).
...
- **Batches too wide to cull.** Each chunk built one instanced mesh per plant kind, spanning the whole valley floor — hundreds of metres across. Its bounding sphere was so large that the frustum test could almost never reject it, so foliage kept drawing long after it left the screen: in the close view, 75% of the foliage triangles submitted were changing under a thousandth of the frame each. Sprawling batches are now halved along their longest axis until each part fits within about 112 m, while compact batches are left whole — splitting those would only add draw calls. Same instances, same geometry, same material, so the picture is unchanged.
- **Background layers in the ambient-occlusion prepass.** Ambient occlusion redraws the whole scene for depth and normals. The far hillside canopy, the distant canopy lumps and the forest undergrowth cover a few per cent of the screen behind fog and nearer trees, and removing them from that prepass alters up to 19% of pixels by an average well under one part in a hundred — invisible at 6× amplification. They now sit it out and are drawn only in the colour pass.

Together, at 412 × 915 and 2.625× in the close view, a jungle frame went from 543 draw calls and 3,066,569 triangles to 538 and 1,049,193: **66% fewer triangles for 1% fewer draw calls**. Chunk build time is unchanged (39 ms median). The coast, desert and alpine routes render identically, and the same batch splitting would be worth roughly 60,000–100,000 triangles a frame there if it is wanted later.
