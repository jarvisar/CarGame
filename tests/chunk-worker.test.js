...
const builders = { coast: CoastalChunk, desert: DesertChunk, snow: SnowChunk, jungle: JungleChunk, plains: PlainsChunk };
...
test('transferred chunks retain geometry, transforms, shaders, bounds and animation', async () => {
  const worker = new Worker(`
    ...
    (async () => {
      ...
      for (const journey of ['coast', 'desert', 'snow', 'jungle', 'plains']) for (const index of [-9, 0, 1, 65, 0]) {
