...
function barn() {
  const p = new Parts();
  p.box([0, 2.1, 0], [7, 4.2, 11], barnRed);
  p.gable([0, 0, 0], 7, 11, 4.2, 7.3, barnRed, shingle);
  for (const z of [-5.5, 5.5]) for (const x of [-3.5, 3.5]) p.box([x, 2.1, z], [.16, 4.2, .16], trim);
  p.box([-3.55, 1.65, 1.6], [.12, 3.3, 3.2], '#3a2c25');
  p.box([-3.62, 1.65, 1.6], [.06, 3.3, .18], trim); p.box([-3.62, 1.65, 3.2], [.06, 3.3, .18], trim);
  p.box([-3.62, 3.3, 1.6], [.06, .16, 3.3], trim); p.box([-3.62, .1, 1.6], [.06, .16, 3.3], trim);
  p.box([-3.55, 5.4, -5.52], [.1, 1.5, 1.3], '#3a2c25');
  p.box([0, 7.6, 0], [1.1, .8, 1.5], barnRed); p.box([0, 8.1, 0], [1.5, .18, 1.9], shingle);
  for (const z of [-3, 4.3]) p.box([3.55, 2.6, z], [.08, 1, 1.2], '#4b3b33');
  return p.finish();
}
...
function farmhouse() {
  const p = new Parts(), wall = '#ece4cf';
  p.box([0, 1.6, 0], [6, 3.2, 7.6], wall);
  p.gable([0, 0, 0], 6, 7.6, 3.2, 5.4, wall, '#6d5f57');
  p.box([1.6, 5.2, -1.8], [.7, 2.2, .7], '#8a5f4a');
  p.box([-3.9, 1.32, 0], [1.9, .18, 7.6], '#c9bda2'); p.box([-3.9, .8, 0], [1.9, .9, 7.6], '#b7ab90');
  p.box([-4.75, 2.85, 0], [1.8, .12, 7.8], '#6d5f57', [0, 0, -.16]);
  for (const z of [-3.5, -1.2, 1.2, 3.5]) p.box([-4.7, 2.15, z], [.12, 1.7, .12], wall);
  p.box([-3.03, 1.5, -2.2], [.06, 2, .9], '#5a4a40');
  for (const z of [.6, 2.6]) p.box([-3.03, 1.85, z], [.06, 1, .8], '#40515c');
  for (const z of [-2.4, 0, 2.4]) p.box([3.03, 1.85, z], [.06, 1, .8], '#40515c');
  for (const x of [-1.8, 1.8]) p.box([x, 1.85, 3.83], [.8, 1, .06], '#40515c');
  return p.finish();
}
