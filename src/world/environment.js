...
class Chunk {
  ...
  buildTerrain() {
    ...
    this.addMesh(geometryFrom(positions, colors), terrainMaterial, true);
  }
  buildWater() {
    ...
    const seaPoint = (row, col) => {
      ...
      const u = -420 + col * 16 + (randomAt(row + 122, col) - .5) * 9;
  ...
  buildScenery() {
    ...
    const trunks = [], foliage = [], shrubs = [], rocks = [], posts = [], caps = [];
    ...
    for (let patch = 0; patch < 16; patch++) {
      ...
      for (let flower = 0; flower < 6; flower++) {
        const p = positionAt(s + (random() - .5) * 3, u + (random() - .5) * 2);
