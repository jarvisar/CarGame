...
export function createTrafficModels() {
  ...
  const templates = TRAFFIC_MODELS.map(spec => {
    ...
    const { width: w, length: l, cabin: [cw, ch, cl], cabinZ: cz, name } = spec;
    const roofY = 1.22 + ch;
    box([w, .65, l], [0, .89, 0]);
    ...
    for (const side of [-1, 1]) {
      ...
      for (const z of [-l * .3, l * .3]) {
        const tire = new THREE.CylinderGeometry(.43, .43, .25, 10); tire.rotateZ(Math.PI / 2);
        add(tire, [side * w / 2, .44, z], 'details', '#2b3434');
        const hub = new THREE.CylinderGeometry(.21, .21, .26, 8); hub.rotateZ(Math.PI / 2);
        add(hub, [side * w / 2, .44, z], 'details', '#bfc4b9');
      }
    }
  ...
  return {
    ...
    setNight(night) { headlights.emissiveIntensity = night ? 2.3 : .3; taillights.emissiveIntensity = night ? 1.8 : .25; },
    dispose() {
